...
### 인터페이스와 추상클래스의 차이
#### 인터페이스는 기존 클래스가 구현하게 만들기가 쉽지만 추상클래스는 어렵다.
#### 새로 추가된 추상클래스를 모든 하위클래스가 상속받아야 하기 때문에
### 믹스인정의 : 클래스의 주된 목적 이외에도 선택적인 부가기능을 추가하는것.
#### 클래스의 주된 기능에 선택적 기능을 혼합한다고 헤서 Mix in 이라고 부른다(Ex. Compareble 인터페이스)
### 인터페이스도 상위 인터페이스를 상속 받는것이 가능함. 다중 상속도 가능함
#### ex) public interface SingerSongWriter extends Singer, SongWriter {}
### 인터페이스 + 추상 골격 클래스 = 템플릿 메소드 패턴
### Object의 메서드들을 인터페이스에서 왜 디폴트 메서드로 제공하면 안될까?

## Item 21. 인터페이스는 구현하는 쪽을 생각해 설계하라
### 디폴드 메서드의 추가로 인터페이스를 수정하여도 컴파일 오류가 나지 않게 할수있지만
### 이게 꼭 기존 클래스들의 정상 동작을 보장하는 것은 아니다

## Item 22.인터페이스는 타입을 정의하는 용도로만 사용하라
### 인터페이스는 자신의 인스턴스로 무슨일을 할수있는지 클라이언트에 얘기해주는 역할
### 상수 인터페이스 패턴(상수로만 이루어진 인처페이스) 사용해서는 안되는 안티패턴
#### 내부구현이 외부에 공개 되어 버리고 심하게는 클라이언트의 코드가 이 상수에 의존이 되어 버림
#### 상수를 공개할 목적이면 그 상수가 쓰일 클래스에 추가하거나 인스턴스화가 불가능한 유틸 클래스를 만들어 넣으면 됨
### JAVA7부터 숫자 리터럴에 _를 사용하면 숫자값에는 영향을 미치지 않으면서 가독성을 높일수있다.

## Item 23. 태그달린 클래스보다는 클래스 계층구조를 활용하라
### 서브타이핑을 활용하자
#### 하나의 클래스에서 여러 타입을 표현하려면 문제가 많이 생긴다
#### 따라서 공통이되는 타입을 루트 클래스로 올리고 하위 클래스에서 구체적인 타입을 정하면
#### 코드량도 줄고 유연성도 높아진다

## Item 24. 멤버 클래스는 되도록 static으로 만들라
### 정적 멤버클래스와 비정적 멤버 클래스의 차이
#### 정적 멤버클래스는 바깥 클래스의 인스턴스와 독립적이지만
#### 비정적 멤버클래스는 바깥 클래스의 인스턴스와 암묵적으로 연결되어있다
### (비정적) 멤버 클래스
#### 주로어댑터를 정의 할때 자주 쓰인다. 즉 어떤 클래스의 인스턴스를 감싸 마치 다른 클래스처럼 보이게 할때
##### ex) Iterator: 바깥 클래스 안에 Iterator를 정의하여 반복 작업이 필요할때 Iter클래스를 뷰로 리턴
#### 비정적멤버클래스와 바깥 클래스의 관계는 멤버클래스가 인스턴스화 될때 정해진다.
#### 보통은 바깥클래스에서 멤버클래스의 생성자를 호출할때 정해지지만
#### 드물게 바깥인스턴스의 클래스.new MemberClass()를 호출하여 만들수도있다
#### 이 관계정보는 비정적멤버클래스의 인스턴스안에 저장되어 메모리 공간을 차지하며 생성속도도 더 걸린다.
##### 정규화된 this로(클래스명.this) 바깥 클래스의 인스턴스에 연결할 수 있다.
### 정적 멤버 클래스
#### 멤버 클래스에서 바깥 클래스의 인스턴스를 접근할 일이 없다면 무조건 static을 붙여야한다
#### static을 안쓰면 인스턴스 관계 설정에 필요한 메모리르 더 잡아먹고
#### 가비지 콜렉터가 바깥 클래스의 인스턴스를 회수하지 못한다.
#### 사용예> Map에는 key,Value 쌍으로 이루어진 엔트리들이 있지만 이 엔트리들은 Map을 직접 참조하여 사용하는 일이 없다
#### 따라서 Entry는 정적 멤버 클래스로 만드는것이 좋다
### 익명(중첩)클래스
#### Test t = new test {익명클래스}
#### 위와같이 사용됨 정적 팩터리 메서드를 구현할때 쓰임
### 지역클래스
#### 메소드 내부에서 선언, 네가지 중첩 클래스중 가장 드물게 사용됨
#### 유효범위는 지역변수와 같고 비정적 문맥에서 사용될때만 바깥 인스턴스를 참조할 수 있다.
