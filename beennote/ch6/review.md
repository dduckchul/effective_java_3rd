...
* @Override 애너테이션을 읽관되게 사용하지 않을 경우 overriding한 게 아니라 overloading되는 경우도 있다.
* 상위 클래스의 메서드를 재정의하려는 모든 메서드에 @Override 애너테이션을 달자.
* 예외는 한 가지뿐이다. 구체 클래스에서 상위 클래스의 추상 메서드를 재정의할 때는 굳이 @Override를 달지 않아도 된다. -> 컴파일러가 그 사실을 알려주기 때문.
* 추상 클래스나 인터페이스에서는 상위 클래스나 상위 인터페이스의 메서드를 재정의하는 모든 메서드에 @Override를 다는 것이 좋다.

## 아이템41 / 정의하려는 것이 타입이라면 마커 인터페이스를 사용하라

#### 마커 인터페이스
* 아무 메서드도 담고 있지 않고, 단지 자신을 구현하는 클래스가 특정 속성을 가짐을 표시해주는 인터페이스를 마커 인터페이스라 한다. ex) Serializable

#### 마커 인터페이스 > 마커 애너테이션의 경우
...
#### 적용 시점
* 클래스와 인터페이스 외의 프로그램 요소에 마킹해야 할 때 애너테이션을 쓸 수 밖에 없다.
